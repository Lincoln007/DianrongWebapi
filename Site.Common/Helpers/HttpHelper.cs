...
namespace Site.Common.Helpers
{
    ...
    {
        private static ILogger logger =LogManager.GetLogger("req");
        /// <summary>
        /// 获取返回的结果
        /// </summary>
        ...
        public static async Task<T> GetResultAsync<T>(HttpReqModel reqModel) where T : class
        {
            ...
            logger.Info($"返回{rspString.RemoveEmptyChar()}");
            try
            {
            ...
            using (var httpClient = new HttpClient(handler))
            {
                ...
                if (reqModel.ReqData != null)
                {
                    foreach (var prop in reqModel.ReqData.GetType().GetProperties())
                    {
                        ...
                        {
                            propValue = Uri.EscapeUriString(propValue);
                ...
                {
                    var postContent = new StringContent(JsonConvert.SerializeObject(reqModel.ReqBody ?? new { }), Encoding.UTF8, "application/json");
